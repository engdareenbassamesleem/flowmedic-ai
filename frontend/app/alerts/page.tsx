...
export default function AlertsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Alerts"
        description="Persistent alert decisions and safe local delivery attempts. Rules are disabled by default."
      />
      {error ? (
        <StatePanel
        ...
        <div className="space-y-6">
          <section className="panel overflow-hidden">
            ...
            {(rules.data?.data.length ?? 0) === 0 ? (
              <StatePanel title="No alert rules configured" message="Create an explicitly enabled mock rule through the API to record local delivery attempts." />
            ) : (
              <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Rule</th><th>Trigger</th><th>State</th><th>Cooldown</th><th>Provider</th></tr></thead><tbody>{rules.data?.data.map((rule) => <tr key={rule.id}><td><strong>{rule.name}</strong></td><td>{labels[rule.trigger_type]}</td><td><span className={rule.enabled ? "badge badge-success" : "badge"}>{rule.enabled ? "Enabled" : "Disabled"}</span></td><td>{rule.cooldown_seconds}s</td><td>Local mock</td></tr>)}</tbody></table></div>
            )}
          </section>
          <section className="panel overflow-hidden">
            <div className="panel-header"><div><h2>Alert history</h2><p>Mock delivery is recorded locally and does not contact an external channel.</p></div></div>
